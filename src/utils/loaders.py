...
class ImageLabelLoader:
    ...
    """
    def __init__(self, image_folder, target_size, x_col="image_id"):
        """
        Class constructor.
    ...
    def build(self, csv_data, batch_size, label=None):
        ...
        if label:
            # Generator of amount_of_images/batch_size tuples where (image_batch, y_col).
            ...
            # is the same image batch as first item a.k.a (batch_size, w, h, c), so (image_batch, image_batch).
            data_flow = data_gen.flow_from_dataframe(
                csv_data,
                self.image_folder,
                x_col=self.x_col,
